...
public class DefaultWizardPage extends WebPage
{
	...
	public DefaultWizardPage()
	{
		...
		final UserWizard wizard = new UserWizard("wizard", "Create a user") {
			...
			protected void onFinish(AjaxRequestTarget target)
			{
				...
				user.setPhone("1212312312");
				user.setRole(UserRole.ADMIN);
				this.info(String.format("Created user: '%s' - %s [%s]", user.getFirstName(), user.getPhone(), user.getRole()));
		...
		form.add(new AjaxButton("open") {
			...
			{
				wizard.setModelObject(new UserProfile());
				wizard.open(target);
			}
		});
	}
	 ...
	 */
	abstract class UserWizard extends AbstractWizard<UserProfile>
	{
		private static final long serialVersionUID = 1L;
		...
		protected IModel<?> initModel()
		{
			...
			return new CompoundPropertyModel<UserProfile>(new Model<UserProfile>());
		}
		...
		class Step3 extends StaticContentStep
		{
			...
			protected void onConfigure()
			{
				...
				UserProfile user = UserWizard.this.getModelObject();

				StringBuilder builder = new StringBuilder("<ul>");
				builder.append("<li>").append("User fname: ").append(user.getFirstName()).append("</li>");
				builder.append("<li>").append("User lname: ").append(user.getLastName()).append("</li>");
				builder.append("<li>").append("User role: ").append(user.getRole().toString()).append("</li>");
				builder.append("</ul>");
